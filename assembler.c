...
/*
                                id 208685784, add ori's
				In this assignment we are implementing...
								                                                                        */
/********************************************************************************************************/
...
static void command_to_output(FILE *output_file, char *line)
{
    char opcode[MAX_OPCODE_LEN], rd[MAX_REG_LEN], rs[MAX_REG_LEN], rt[MAX_REG_LEN], imm[/*complete correct size*/];
    sscanf_s(line, " %[^ $] $%[^,], $%[^,], $%[^,], %s ",  
        opcode, MAX_OPCODE_LEN,
        rd, MAX_REG_LEN,
        rs, MAX_REG_LEN,
        rt, MAX_REG_LEN, 
        imm, /*complete correct size*/);
    ...
    fprintf(output_file, "%02X%01X%01X%01X%05X\n", opcode_d, rd_d, rs_d, rt_d, imm_d);
}
