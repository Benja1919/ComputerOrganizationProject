MAIN:
	add $a0, $zero, $imm, 0						# set 1st sector adress
...
Disk_test:
	...
	out $zero, $imm, $zero, 16					# ioregister[16 = diskbuffer] = $s0 = 0 -> start with first memory cell
...
First_loop:
	lw $t1, $zero, $t0, 0						# load the i'th word from memory (after DMA from sector)
...
Sec_read:
	jal $ra, $imm, $zero, Wait
	out $a1, $imm, $zero, 15					# ioregister[15 = disksector] = $a0 -> select the first sector given
	out $s0, $imm, $zero, 14					# ioregister[14 = diskcmd] = 1 -> read
